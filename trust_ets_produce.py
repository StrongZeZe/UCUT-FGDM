...
def generate_confidence_trust_data(k_values_list, position):
    ...
    for batch_idx, k_values in enumerate(k_values_list):
        ...
        for k in k_values:
            ...
            # 根据条件调整自信水平
            if confi >= 0.9:
                final_confi = confi - random.uniform(0.01, 0.1)
            elif 0.3 < confi <= 0.5:
                final_confi = confi + random.uniform(0.01, 0.1)
            elif confi <= 0.3:
                final_confi = 0.3 + random.uniform(0.05, 0.2)
            else:
                final_confi = confi

            # 确保在合理范围内
            final_confi = max(0.1, min(0.95, final_confi))
            confidences.append(final_confi)
        ...
        for i in range(n_dms):
            ...
            while len(samples) < n_dms and attempts < max_attempts:
                sample = np.random.normal(mean_confidence, 0.5)  # 使用标准差为0.5
                # 确保样本在[0.1,0.95]范围内
                if 0.3 <= sample <= 0.95:
                    ...
                    samples.append(0.3+random.uniform(-0.05,0.05))
                attempts += 1

            # 如果无法生成足够的样本，使用随机值
            while len(samples) < n_dms:
                samples.append(random.uniform(0.1, 0.95))

            confidence_matrix[i] = samples
        ...
        # 保存Excel文件
        excel_path = os.path.join(position, f"trust_ets_{batch_idx + 1}.xlsx")
        save_to_excel(batch_result, excel_path)

        # 保存pickle文件
        pkl_path = os.path.join(position, f"trust_ets_{batch_idx + 1}.pkl")
...
def main():
    ...
    # 保存所有结果
    all_results_path = os.path.join(position, "all_trust_ets_results.pkl")
