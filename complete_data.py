...
#设置是否超时
def utility_best_with_timeout(sure_Hs, confi_trust_sets, trust_M, Qvalue, GCD, index_weight, timeout=30):
...
if __name__ == "__main__":
    ...
    # break_ij_record = [[] for _ in range(5) ]                                                          #记录因为系统卡顿导致无法输出的大循环i与小循环j
    # 打包,用于后续分析
    position1 = r"E:\newManucript\python_code_rare\script2\data\simulation_data\test1"
    ...
    #position1 = r"E:\newManucript\python_code_rare\script2\data\test1"
    #position3 = r"E:\newManucript\python_code_rare\script2\data\test3"
    for i in range(0,4):                                                                                 #补足3个实验
        assessment_file = [i for i in range(5)]
        trust_file = [i for i in range(5)]

        for items in range(5):
            assessment_file[items] = os.path.join(position, f"table_values_{items+1}.pkl")  # 命名
            if i==0:
                trust_file[items] = os.path.join(position, f"trust_ets_{items+1}.pkl")  # 读取第一次实验的自信-信任与置信信息
            elif i==1:
                trust_file[items] = os.path.join(position, f"trust_ets_random{items+1}.pkl")  # 读取第二次实验的自信-信任与置信信息
            elif i==2:
                trust_file[items] = os.path.join(position, f"trust_ets_True{items+1}.pkl")  # 读取第三次实验的自信-信任与置信信息
            elif i==3:
                trust_file[items] = os.path.join(position, f"trust_ets_none-confi_{items+1}.pkl")  # 读取第三次实验的自信-信任与置信信息

        #读取实验的结果
        original_file = os.path.join(eval("position"+str(i+1)), f"break_ij_{i+1}.pkl")
        original_file2 = os.path.join(eval("position"+str(i+1)), f"group_weight_{i+1}.pkl")
        original_file3 = os.path.join(eval("position"+str(i+1)), f"feedback_Num_{i+1}.pkl")
        original_file4 = os.path.join(eval("position"+str(i+1)), f"group_CI_{i+1}.pkl")
        original_file5 = os.path.join(eval("position"+str(i+1)), f"group_utility_loss_{i+1}.pkl")
        original_file6 = os.path.join(eval("position"+str(i+1)), f"GCD_final_name_{i+1}.pkl")
        original_file7 = os.path.join(eval("position"+str(i+1)), f"Group_H_name_{i+1}.pkl")
        original_file8 = os.path.join(eval("position"+str(i+1)), f"DMs_H_{i+1}.pkl")
        original_file9 = os.path.join(eval("position"+str(i+1)), f"C_T_FINAL_{i+1}.pkl")
        ...
        break_ij_record_pre = original_data
        group_weight_list = original_data2
        feedback_list = original_data3
        group_CI_list = original_data4
        group_utility_list = original_data5
        GCD_final_list = original_data6
        GROUP_H_list = original_data7
        DMs_H_list = original_data8
        C_T_FINAL_list = original_data9
        break_ij_record=[]          #记录这一次的,再运行完之后，可以通过调试break_ij_record_pre观察还有哪些没跑完
        for ss in range(len(break_ij_record_pre)):
            none_element=break_ij_record_pre[ss]
            # none_element[0]即为第几个大循环，none_element[1]即为第几个小循环.
            if len(none_element)==0:
                print("跳出")
                continue
            else:
                with open(assessment_file[none_element[0]], 'rb') as f:
                    assessment_value = pickle.load(f)
                with open(trust_file[none_element[0]], 'rb') as f:
                ...
                cl_index, needModify, GCD, groupIndexweight, Qvalue, solves_Message, index_weight, sure_Hs, group_Hs = groupdecision(
                        assessment_value[none_element[1]], len(w_true), 0.9)
                # 自信分布
                if i==0 or i==3:
                    av_conf, st_conf, trust_M = confidance_pmt(cl_index, assessment_value[none_element[1]], trust_value, hfpr_list,
                                                               hflpr_list,
                                                               fuz_unc_a)  # fuz_unc_a为明确模糊度与不确定度权重的参数a
                else:
                    av_conf, st_conf, trust_M = confidance_pmt(cl_index, assessment_value[none_element[1]], trust_value[none_element[1]], hfpr_list,
                                                               hflpr_list,
                                                               fuz_unc_a)  # fuz_unc_a为明确模糊度与不确定度权重的参数a
                num_ets = len(assessment_value[none_element[1]])  # 决策者个数
                trust_post_par, Trust_set, confidance_set = trust_func(trust_M, st_conf, a_gama_T, num_ets,
                                                                       index_weight, sure_Hs,
                                                                       assessment_value[none_element[1]])  # 对trust_M进行了修改
                ...
                while True:
                    ...
                    if minGw is None:
                        ...
                        if flag_main_utility < 10:
                            continue
                        else:
                            break_ij_record.append([none_element[0], none_element[1]])  # 记录这一次的失败
                            break
                    if isinstance(minGw, (list, N.ndarray)):  # 判断是否为求和类型
                        ...
                        if GCD_minULs < 0.9:
                            ...
                            tconfi_trust_sets=trust_modi
                            continue
                        else:  # 则正常跑通，跳出
                            break
                    else:
                        print("第", none_element[0], "次大循环的第", none_element[1], "次循环发生时间卡滞错误，重新来一次")
                        print("第", times_out_num, "次重来")
                        times_out_num = times_out_num + 1
                        if times_out_num == 5:  # 只循环4次
                            break_ij_record.append([none_element[0], none_element[1]])  # 记录这一次的失败
                            break
                        continue                #发生错误，重新迭代跑

                # if ss==0:             因为进程问题，故在外部运行
                #     FLAG_FEEDBACK_NUM=1
                #     minGw=[0.15030017692908507, 0.2562975011004168, 0.21250729744921995, 0.13304596171223187, 0.09260862487249487, 0.15524043793655146]
                #     minGcl=0.9967050570057036
                #     minGObjec=0.10501483888320129
                #     GCD_minULs=0.9820194453931413
                #     GCD_HH = [[0.3702886607876992, 0.416704215933737, 0.5188185540120597, 0.6248789784752271,
                #                0.49999996413839254],
                #               [0.5483546165479936, 0.6498184309514521, 0.7440180173702867, 0.6297116362445644],
                #               [0.6245122415201639, 0.6856815365876895, 0.5865081477683519],
                #               [0.5850146726606993, 0.4605333379220476], [0.38694343557263955]]
                #     finalHH = [[[0.3802616039484243, 0.40975290449377444, 0.5512115934780457, 0.6138409792212381,
                #                  0.4999999785969918],
                #                 [0.5306658584823841, 0.6680560419555885, 0.7244215347776717, 0.6197385463628726],
                #                 [0.6548262487951222, 0.6839823178570832, 0.5933881962902785],
                #                 [0.5464629335271771, 0.43778235582546143], [0.43364146660512737]], [
                #                    [0.33791658014710957, 0.40730673224810815, 0.5094314713348491, 0.6117029904507023,
                #                     0.49999998395540923],
                #                    [0.5743056268587953, 0.6720648997831697, 0.7579083378333215, 0.6620843296342185],
                #                    [0.6121458933313951, 0.6928255067983999, 0.593725358221882],
                #                    [0.5914259732032054, 0.4808941571017496], [0.3944508608863735]], [
                #                    [0.37998095399327364, 0.4561259040294896, 0.5095917094787262, 0.6163952433043737,
                #                     0.4999998976331505],
                #                    [0.5771306166982177, 0.6300857374425538, 0.731273188852382, 0.6200190533207834],
                #                    [0.6174002661092672, 0.6461603344440984, 0.5992215753063265],
                #                    [0.5934323049388558, 0.4811667044407435], [0.39006529032701986]], [
                #                    [0.3802516463621859, 0.4087116664129852, 0.5150848101156664, 0.6192601766498832,
                #                     0.49999997439010957],
                #                    [0.5294488412784122, 0.6353474184608031, 0.7327559153406908, 0.6197484884496905],
                #                    [0.6223681575442299, 0.6572441038996496, 0.5953063787656231],
                #                    [0.5520312784628071, 0.47202518572696595], [0.39145713536298743]], [
                #                    [0.3812491889265784, 0.4038234311889269, 0.509530928836742, 0.6720108442207257,
                #                     0.49999998111940963],
                #                    [0.5234411609791527, 0.6372168194343835, 0.7747519465909376, 0.6187509381628141],
                #                    [0.6174996428390243, 0.755633484248975, 0.5439264672885793],
                #                    [0.650659087839676, 0.4215447556468224], [0.3141660867140592]]]
                #     min_c_t_set_mody = [
                #         [[0.9701995288048015], [0.6414916672518344], [0.6317984187205049], [0.7493673056832895],
                #          [0.4892117736429382]],
                #         [[0.6590903532948282], [1.0], [0.6429924797127726], [0.6353800991636516], [0.5960865538728976]],
                #         [[0.6556096958226308], [0.6637102077700351], [0.9986186592196922], [0.7514996567756874],
                #          [0.5778114141617741]],
                #         [[0.8055143234055203], [0.6954340143106855], [0.8045692257502669], [0.7682133091897665],
                #          [0.6335866154725334]],
                #         [[0.6159938528164479], [0.6785732558358843], [0.657628822379437], [0.6671962498600352],
                #          [0.7135596138303356]]]
                #
                #     feedback_list[none_element[0]][none_element[1]]=FLAG_FEEDBACK_NUM
                #     group_weight_list[none_element[0]][none_element[1]]=minGw           # 存储最终群共识的权重集合
                #     group_CI_list[none_element[0]][none_element[1]]=minGcl              # 存储最终的群决策一致水平结果集合
                #     group_utility_list[none_element[0]][none_element[1]]=minGObjec      # 存储最终的效用降低水平集合
                #     GCD_final_list[none_element[0]][none_element[1]]=GCD_minULs         # 存储最终的群共识集合
                #     GROUP_H_list[none_element[0]][none_element[1]]=GCD_HH               # 存储最终的群共识矩阵的集合
                #     DMs_H_list[none_element[0]][none_element[1]]=finalHH                # 存储最终各个决策者最终的决策矩阵
                #     C_T_FINAL_list[none_element[0]][none_element[1]]=min_c_t_set_mody  # 存储最终各个决策者间的自信与信任
                # if ss==1:
                #     FLAG_FEEDBACK_NUM = 1
                #     minGw = [0.13262650023166428, 0.23225108613148127, 0.21256623910387323, 0.1358597322217579, 0.1339459656213533, 0.15275047668987007]
                #     minGcl = 0.9966277889317408
                #     minGObjec = 0.18010117828120664
                #     GCD_minULs = 0.9752058155674287
                #     GCD_HH = [[0.36001585726078406, 0.38040837197950633, 0.4814186795735803, 0.49865869075992764,
                #                0.4603858253930717],
                #               [0.5207281817663991, 0.6217662252275467, 0.6434330808287746, 0.599102243061427],
                #               [0.6037133075475765, 0.6037133041287778, 0.5808368280013693],
                #               [0.4999999709166545, 0.45905348134814666], [0.4789671458167126]]
                #     finalHH = [[[0.37947539578826794, 0.392917061460833, 0.49405734197190465, 0.5020750087691958,
                #                  0.43075612500883226],
                #                 [0.5143943871989972, 0.6150726801636738, 0.6761798463128508, 0.5509497590320344],
                #                 [0.6020974710339377, 0.6020974693107827, 0.5378682641626314],
                #                 [0.49999998536347856, 0.4242428769136086], [0.4366987830356354]], [
                #                    [0.37710174878366115, 0.38309722667693624, 0.48745837659194674, 0.495891731181177,
                #                     0.5220255108881411],
                #                    [0.5058362179493784, 0.6106925353452461, 0.6219920038584126, 0.6408325749104887],
                #                    [0.6060667111653097, 0.6060667018391113, 0.6356293272636155],
                #                    [0.4999999211045845, 0.5232538556494231], [0.534567134294835]], [
                #                    [0.3734649935401621, 0.42866933361024423, 0.532902883951533, 0.5808354250682651,
                #                     0.5198490249079941],
                #                    [0.5592474295709827, 0.6562916488272142, 0.6666401679664422, 0.642326449274214],
                #                    [0.6023528921109053, 0.6023528902019776, 0.5882688962288947],
                #                    [0.4999999837487549, 0.47285727259279126], [0.4869461409487355]], [
                #                    [0.3370258037593126, 0.3243308707021868, 0.41741011120881827, 0.42870198360449135,
                #                     0.40644455749531466],
                #                    [0.47993701756566615, 0.5807769844321312, 0.5916951959513809, 0.5686566975057994],
                #                    [0.6026740343007372, 0.6026740319177761, 0.5904032469153352],
                #                    [0.49999997936731, 0.4744556021682722], [0.48903444628467635]], [
                #                    [0.3355716665804403, 0.38673434020077896, 0.4910603995591721, 0.5041221496451754,
                #                     0.43647005908959957],
                #                    [0.5576151524262649, 0.6593667051202919, 0.673244223271959, 0.6038671631275271],
                #                    [0.6058030554267915, 0.6058030535226565, 0.550185089401211],
                #                    [0.49999998378750793, 0.39454780526910327], [0.4454096595283223]]]
                #     min_c_t_set_mody = [
                #         [[0.9992578262300259], [0.6399407360467466], [0.6265627055329169], [0.5612872363030235],
                #          [0.692616121476659]],
                #         [[0.60750512897973], [0.998621271583298], [0.6493265608040627], [0.7338772812746374],
                #          [0.6017490286762297]],
                #         [[0.6449369043059736], [0.6552330801097859], [0.9173319141151534], [0.6431269847265138],
                #          [0.7062864794245831]],
                #         [[0.6593674878761577], [0.8153406429689461], [0.6876450783479227], [0.7030000000000001],
                #          [0.7082988765123364]],
                #         [[0.7182655881353367], [0.590868162690246], [0.7324318116797218], [0.6222305630503354],
                #          [0.8377989987136527]]]
                #
                #     feedback_list[none_element[0]][none_element[1]] = FLAG_FEEDBACK_NUM
                #     group_weight_list[none_element[0]][none_element[1]] = minGw  # 存储最终群共识的权重集合
            ...
            print("___________________________________________________________")
            print("___________________________________________________________")
            print("完成第",i,"个实验第",none_element[0],"个大循环的第",none_element[1],"个小循环")

        feedback_name=os.path.join(eval("position"+str(i+1)),f"feedback_Num_{i+1}.pkl")
        group_weight_name = os.path.join(eval("position"+str(i+1)),f"group_weight_{i+1}.pkl")  # 存储最终群共识的权重集合
        group_CI_name = os.path.join(eval("position"+str(i+1)),f"group_CI_{i+1}.pkl")  # 存储最终的群决策一致水平结果集合
        group_utility_name = os.path.join(eval("position"+str(i+1)),f"group_utility_loss_{i+1}.pkl")  # 存储最终的效用降低水平集合
        GCD_final_name = os.path.join(eval("position"+str(i+1)),f"GCD_final_name_{i+1}.pkl")  # 存储最终的群共识集合
        GROUP_H_name = os.path.join(eval("position"+str(i+1)),f"GROUP_H_name_{i+1}.pkl")  # 存储最终的群共识矩阵的集合
        DMs_H_name = os.path.join(eval("position"+str(i+1)),f"DMs_H_{i+1}.pkl")  # 存储最终各个决策者最终的决策矩阵
        C_T_FINAL_name = os.path.join(eval("position"+str(i+1)),f"C_T_FINAL_{i+1}.pkl")  # 存储最终各个决策者间的自信与信任
        break_ij_record_name = os.path.join(eval("position" + str(i+1)),f"break_ij_{i+1}.pkl")  # 存储最终各个决策者间的自信与信任
        ...
        # original_file = os.path.join(position2, f"table_values_1.pkl")
        # with open(original_file, 'rb') as f:
        #     original_data = pickle.load(f)
        #
        # original_file2 = os.path.join(position2, f"table_values2_1.pkl")
        # with open(original_file, 'rb') as f:
        #     original_data2 = pickle.load(f)
        #
        # array_file = os.path.join(position, f"assessment_array_1.pkl")
        ...
        #     soleve = solveModel(original_data[i], 6)
        #     soleve2 = solveModel(original_data2[i], 6)
        #     end_array.append(soleve)
        #     end_array2.append(soleve)
        #
        # #print(end_array)
        # # 确保输入都是 numpy 数组
        # if isinstance(array_data, list):
        #     array_data = N.array(array_data)
        # if isinstance(end_array, list):
        #     end_array = N.array(end_array)
        # if isinstance(end_array2, list):
        #     end_array2 = N.array(end_array2)
