...
fig_width = 3.5  # 单栏宽度（英寸） - 符合SCI要求
fig_height = 3.0  # 紧凑高度
base_fontsize = 10  # 基础字体大小

# 加载数据
position3 = r"E:\newManucript\python_code_rare\script2\data\simulation_data\sensibility1"
ariginal_file = os.path.join(position3, f"GCD_final_name_1.pkl")
...
line = ax.plot(shape_params, list_data,
               ...
               marker='o', markersize=3.0, markeredgecolor='black',
               markeredgewidth=0.3, markerfacecolor='white',
               alpha=0.9, zorder=3)

# 添加网格 - 使用更细的网格线
ax.grid(True, which='both', linestyle='--', linewidth=0.4, alpha=0.6, zorder=1)

# 设置坐标轴标签
ax.set_xlabel('Shape Parameter', fontsize=base_fontsize, labelpad=6)
ax.set_ylabel('GCD', fontsize=base_fontsize, labelpad=6)

# 设置坐标轴范围
ax.set_xlim([0, 10])
# 设置y轴范围，留出一些边距
y_min = np.min(list_data) - 0.005
y_max = np.max(list_data) + 0.005
ax.set_ylim([y_min, y_max])

# 设置刻度标签字体大小
ax.tick_params(axis='both', which='major', labelsize=base_fontsize-1, pad=3)
...
if len(list_data) < 30:  # 如果数据点较少，可以标记所有点
    ...
    ax.annotate(f'Max: {list_data[max_idx]:.3f}',
                ...
                xytext=(25, 25), ha='center', fontsize=base_fontsize,
                arrowprops=dict(arrowstyle="->", color='red', lw=1.5))

    ax.annotate(f'Min: {list_data[min_idx]:.3f}',
                (shape_params[min_idx], list_data[min_idx]),
                textcoords="offset points",
                xytext=(-30, -20), ha='center', fontsize=base_fontsize,
    ...
    ax.annotate(f'End: {list_data[-1]:.3f}',
                ...
                textcoords="offset points",
                xytext=(-10, 10), ha='right', fontsize=base_fontsize)
...
# 保存图像 - 使用高DPI以确保印刷质量
save_path = r"E:\newManucript\manuscript2\image\12yue\Fig6_GCD.png"
...
save_path_double = r"E:\newManucript\manuscript2\image\12yue\Fig6_GCD_double.png"
plt.savefig(save_path_double, dpi=600, bbox_inches='tight', facecolor='white', pad_inches=0.04)
