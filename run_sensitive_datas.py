                    ...
                    for j in range(i-1, columnNum-1):
                        ...
                        for m in range(len(itemlist)):
                            if bool(itemlist[m])==True:
                                if float(itemlist[m]) <= 1 and float(itemlist[m]) >= 0:
                                    itemlist2.append(float(itemlist[m]) )                    ## 将引号去除，变为原始的float型
    ...
    if data:
        if(len(data)-1)==num_et:
            ...
            for sheet, content in data.items():
                ...
                #年龄
                if float(content[1][1])<20:
                    etB[flag_index].append(1)
                elif float(content[1][1])<30 and float(content[1][1])>=20:
                    etB[flag_index].append(2)
                elif float(content[1][1])<40 and float(content[1][1])>=30:
                    etB[flag_index].append(3)
                elif float(content[1][1])<50 and float(content[1][1])>=40:
                    etB[flag_index].append(4)
                elif float(content[1][1])<60 and float(content[1][1])>=50:
...
#确定自信参数,这里将置信度全部设置为1，用于考虑纯白盒信息去情况
def confidance_pmt2(cl,tableValue,trust_z_i,HFPRLIST,HFLPRLIST,f_u_a):                                           #输入的是groupdecision()传输出的cl,solves_Message以及初始的决策者间信任-置信矩阵
    average_confidance=[[] for _ in range(len(cl))]                     #构建一个与cl的结构相同的数组
    standard_confidance=[[] for _ in range(len(cl))]
        ...
        for i in range(len(C_T_set)):
            for j in range(len(C_T_set[i])):
                if i != j:
                    ...
                    none_know_FC_List[i][j].append([padPrimal[modi_flag] ,1 - sure_zone-padPrimal[modi_flag]])          #黑盒中的自信与信任
                    ...
                    none_know_FC_List[i][j].append([1 - float(trust_modify[i][j][1]) + float(trust_modify[i][i][1]) - float(
...
def feedback(final_Trus, FinalDMH, Final_GroupH, Final_GCD):
    ...
    # 2. 计算距离均值
    mean_Di_max = N.max(Di_list) if Di_list else 0
    mean_Di = mean_Di_max * 3 / 4
    ...
    for m in range(n_decisions):
        ...
        if Dm > mean_Di:
            ...
            adjustment_factor = math.exp(
                -(4 / 3) * (1 / self_trust) * (Dm / Final_GCD) * ((n_decisions - mini_num) / (n_decisions)))
...
if __name__ == "__main__":
    ...
    #flag_test_list=[1,2,3,4]                                                     #用第2个实验作敏感性分析，第一个实验是对形状参数的敏感性分析，第二个实验是对GCD的敏感性分析
    flag_test_list = [2]
    ...
    # 设置逆伽马分布的形状参数为0.5
    a_gama_T_list = N.linspace(start=0.01,stop=10,num=200,endpoint=True).tolist()
    GCD_LIST=N.linspace(start=0.925,stop=0.99,num=10,endpoint=True).tolist()
    # 打包,用于后续分析
    ...
    for flag_test in flag_test_list:
        ...
        no_know_info_record_name = os.path.join(eval("position" + str(flag_test)),
                                            f"no_know_infor_{flag_test}.pkl")  # 存储最终各个决策者间的自信与信任

        #进入每次的小循环
        if flag_test==1:                    #第一次实验
            feedback_list_son = []
            ...
            for a_gama_T in a_gama_T_list:
                ...
                while True:
                    ...
                    if minGw is None:
                        ...
                        print("形状参数取",a_gama_T,"，主程序超时，重新第", flag_main_utility, "次计算")
                        if flag_main_utility < 5:
                            continue
                        else:
                            break_ij_record.append([a_gama_T])  # 记录这一次的失败
                            break
                    if isinstance(minGw, (list, N.ndarray)):  # 判断是否为求和类型
                        ...
                        if GCD_minULs < 0.9:
                            no_feedback_flag = 0
                            print("形状参数取",a_gama_T,"时发生达不到GCD的阈值，开始调试")
                            ...
                            a_gama_T_record.append(a_gama_T)
                            break
                    else:
                        print("形状参数取",a_gama_T,"时发生时间卡滞错误，重新来一次")
                ...
                print("___________________________________________________________")
                print("___________________________________________________________")
                print("完成形状参数取",a_gama_T,"的实验")

            a_gama_T_name = os.path.join(eval("position" + str(flag_test)),f"a_gama_T_{flag_test}.pkl")  # 存储最终各个决策者间的自信与信任

            save_table_values(feedback_list_son,feedback_name)
            save_table_values(group_weight_list_son, group_weight_name)
            save_table_values(group_CI_list_son, group_CI_name)
            save_table_values(group_utility_list_son, group_utility_name)
            save_table_values(GCD_final_list_son, GCD_final_name)
            save_table_values(GROUP_H_list_son, GROUP_H_name)
            save_table_values(DMs_H_list_son, DMs_H_name)
            save_table_values(C_T_FINAL_list_son, C_T_FINAL_name)
            save_table_values(break_ij_record,break_ij_record_name)
            save_table_values(a_gama_T_record, a_gama_T_name)
            continue

        elif flag_test==2:  # 第2次实验
            feedback_list_son = []
            group_weight_list_son = []  # 存储最终群共识的权重集合
            group_CI_list_son = []  # 存储最终的群决策一致水平结果集合
            group_utility_list_son = []  # 存储最终的效用降低水平集合
            ...
            C_T_FINAL_list_son = []  # 存储最终各个决策者间的自信与信任
            break_ij_record = []
            GCD_record = []  # 记录成功取值的a_gama_T
            no_knowledge_infor_record = []  # 记录黑盒信息
            cl_index, needModify, GCD, groupIndexweight, Qvalue, solves_Message, index_weight, sure_Hs, group_Hs = groupdecision(
                TabValue, len_index, 0.9)  # 群共识至少要大于0.9                                #设置GCI的阈值为0.9
            trustH = storeConfidanceValue(filePath2, num_ets)  # num_ets用于进入擦看是否存在决策者未对某决策者做出评价的情况
            av_conf, st_conf, trust_M = confidance_pmt2(cl_index, TabValue, trustH, hfpr_list, hflpr_list,
                                                       fuz_unc_a)  # fuz_unc_a为明确模糊度与不确定度权重的参数a
            a_gama_T = 0.5
            trust_post_par, Trust_set, confidance_set = trust_func(trust_M, st_conf, a_gama_T, num_ets,
                                                                   index_weight, sure_Hs, TabValue)  # 对trust_M进行了修改
            confi_trust_sets = confi_trust(av_conf, st_conf, trust_post_par, Trust_set, confidance_set)
            # 各方案群决策权重，目标1，决策者一致水平偏差，目标2，决策者效用降低，群共识度，群共识矩阵，修改后的各决策者矩阵，最终的信任水平
            print("___________________________________________________________")
            print("___________________________________________________________")
            ...
            while True:
                ...
                if minGw is None:
                    flag_main_utility = flag_main_utility + 1
                    print("GCD取", 0.99, "，主程序超时，重新第", flag_main_utility, "次计算")
                    if flag_main_utility < 10:
                        continue
                    else:
                        break_ij_record.append([0.99])  # 记录这一次的失败
                        break
                if isinstance(minGw, (list, N.ndarray)):  # 判断是否为求和类型
                    sum_minGw = sum(minGw) if isinstance(minGw, list) else N.sum(minGw)
                    sum_not_1 = not N.isclose(sum_minGw, 1.0)  # 浮点数兼容：sum≠1
                    if GCD_minULs < 0.99 or sum_not_1:
                        no_feedback_flag = 0  # 发生了反馈
                        if FLAG_FEEDBACK_NUM > 50:
                            break_ij_record.append([0.99, GCD_minULs, 50])  # 若是存在10次反馈都没成功，跳出
                            break
                        print("GCD取", 0.99, "时发生达不到GCD的阈值，开始调试")
                        print("第", FLAG_FEEDBACK_NUM, "次反馈")
                        FLAG_FEEDBACK_NUM = FLAG_FEEDBACK_NUM + 1  # 计算需要反馈多少次，用于求群决策时间
                        trust_modi = feedback(min_c_t_set_mody, finalHH, GCD_HH,
                                              GCD_minULs)  # trustM,final_Trus,FinalDMH,Final_GroupH,Final_GCD
                        confi_trust_sets = trust_modi
                        GCD = min(GCD_minULs, 0.99)
                        ...
                        DMs_H_list_son.append(finalHH)  # 存储最终各个决策者最终的决策矩阵
                        C_T_FINAL_list_son.append(min_c_t_set_mody)  # 存储最终各个决策者间的自信与信任
                        no_knowledge_infor_record.append(no_knowledge_infor)

                        print('min_c_t_set_mody',
                              [min_c_t_set_mody[0][0], min_c_t_set_mody[1][1], min_c_t_set_mody[2][2],
                               min_c_t_set_mody[3][3], min_c_t_set_mody[4][4]])
                        print('no_knowledge_infor', no_knowledge_infor)
                        print('GCD', GCD_minULs, ',,,,,', 0.99)
                        print('weight', minGw)
                        continue
                    else:  # 则正常跑通，跳出
                        GCD_record.append(0.99)
                        break
                else:
                    print("GCD取", 0.99, "时发生时间卡滞错误，重新来一次")
                    print("第", times_out_num, "次重来")
                    times_out_num = times_out_num + 1
                    if times_out_num == 10:  # 只循环5次
                        break_ij_record.append([0.99])  # 记录这一次的失败
                        break
                    continue
            feedback_list_son.append(FLAG_FEEDBACK_NUM)
            ...
            GCD_name = os.path.join(eval("position" + str(flag_test)),
                                    f"GCD_{flag_test}.pkl")  # 存储最终各个决策者间的自信与信任
            ...
            save_table_values(C_T_FINAL_list_son, C_T_FINAL_name)
            save_table_values(break_ij_record, break_ij_record_name)
            save_table_values(GCD_record, GCD_name)
            save_table_values(no_knowledge_infor_record, no_know_info_record_name)
            ...
            cl_index, needModify, GCD, groupIndexweight, Qvalue, solves_Message, index_weight, sure_Hs, group_Hs = groupdecision(
                TabValue, len_index, 0.9)  # 群共识至少要大于0.9                                #设置GCI的阈值为0.9
            trustH = storeConfidanceValue(filePath2, num_ets)  # num_ets用于进入擦看是否存在决策者未对某决策者做出评价的情况
            av_conf, st_conf, trust_M = confidance_pmt(cl_index, TabValue, trustH, hfpr_list, hflpr_list,
                                                       fuz_unc_a)  # fuz_unc_a为明确模糊度与不确定度权重的参数a
            a_gama_T=0.5
            trust_post_par, Trust_set, confidance_set = trust_func(trust_M, st_conf, a_gama_T, num_ets,
                                                                   index_weight, sure_Hs, TabValue)  # 对trust_M进行了修改
            ...
            while True:
                ...
                if isinstance(minGw, (list, N.ndarray)):  # 判断是否为求和类型
                    ...
                    if GCD_minULs < 0.99 or sum_not_1:
                        ...
                        print("第", FLAG_FEEDBACK_NUM, "次反馈")
                        FLAG_FEEDBACK_NUM = FLAG_FEEDBACK_NUM + 1  # 计算需要反馈多少次，用于求群决策时间
                        trust_modi = feedback(min_c_t_set_mody, finalHH, GCD_HH, GCD_minULs)           #trustM,final_Trus,FinalDMH,Final_GroupH,Final_GCD
                        confi_trust_sets = trust_modi
                        #sure_Hs=finalHH
                        GCD = min(GCD_minULs,0.99)
                        ...
                        print('min_c_t_set_mody',[min_c_t_set_mody[0][0],min_c_t_set_mody[1][1],min_c_t_set_mody[2][2],min_c_t_set_mody[3][3],min_c_t_set_mody[4][4]])
                        print('no_knowledge_infor',no_knowledge_infor)
                        print('GCD', GCD_minULs,',,,,,',0.99)
                        print('weight',minGw)
                        continue
